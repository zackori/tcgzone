...
document.addEventListener("DOMContentLoaded", function () {
  ...
  ------------------------------------------------------ */
  var PRODUCTS = [];
  var isRefreshingProducts = false;
  var inventoryChannel = null;

  function mergeProducts(dbProducts, staticProducts) {
    var staticById = {};
    staticProducts.forEach(function (p) {
      staticById[p.id] = p;
    });
  ...
  function loadProducts() {
    return Promise.all([
      fetch("products.php?ts=" + Date.now(), { cache: "no-store" }).then(
        function (response) {
          if (!response.ok)
            throw new Error(
              "Failed to load products.php (" + response.status + ")",
            );
          return response.json();
        },
      ),
      fetch("products.json?ts=" + Date.now(), { cache: "no-store" }).then(
        function (response) {
          if (!response.ok)
            throw new Error(
      ...
      .catch(function (err) {
        ...
        grid.innerHTML =
          '<p class="text-muted text-center w-100 py-5">Couldn\'t load products. Check that products.php and products.json are both reachable.</p>';
      });
  }
  ...
  function bindInventoryListeners() {
    ...
    window.addEventListener("storage", function (event) {
      if (event.key === "tcgzone_inventory_updated") {
        refreshProducts();
      }
    });

    if (typeof BroadcastChannel !== "undefined") {
      inventoryChannel = new BroadcastChannel("tcgzone_inventory_updates");
      inventoryChannel.addEventListener("message", function (event) {
        if (event.data && event.data.type === "inventory-updated") {
          refreshProducts();
        }
      });
    }
  ...
  var FILTER_LABELS = {
    ...
    price: {
      ...
      "50000-500000": "₱50,000–₱500,000",
      "500000-9999999": "₱500,000+",
    },
  ...
  function getFilteredSortedProducts() {
    var result = PRODUCTS.filter(function (p) {
      ...
      return (
        ...
        searchMatch &&
        (state.filters.cardType === "all" ||
          p.cardType === state.filters.cardType) &&
        ...
        (state.filters.condition === "all" ||
          p.condition === state.filters.condition)
      );
    });
  ...
  function renderGrid() {
    ...
    if (filtered.length === 0) {
      if (state.filters.search) {
        noResultsMsg.textContent =
          'No cards match "' + state.filters.search + '". Try clearing a few.';
      } else {
        noResultsMsg.textContent =
    ...
    grid.innerHTML = pageItems
      .map(function (p) {
        ...
        var stockLabel = p.stock <= 0 ? "Out of Stock" : "Stock: " + p.stock;
        return (
          '<div class="col-6 col-md-4 col-lg-card-5">' +
          '<article class="shop-card" data-id="' +
          p.id +
          '" role="button" tabindex="0" aria-label="View details for ' +
          p.title +
          '">' +
          ...
          p.title +
          "</h3>" +
          '<p class="shop-card-subtitle">' +
          p.subtitle +
          "</p>" +
          '<div class="shop-card-prices">' +
          '<div class="shop-card-price-column"><span class="price-label">Listing Price:</span><p class="listing-price"><strong>' +
          formatCurrency(p.price) +
          "</strong></p></div>" +
          '<div class="shop-card-price-column"><span class="price-label">Market Price:</span><p class="market-price"><strong>' +
  ...
  function getChipLabel(group, value) {
    ...
    if (group === "cardType" || group === "rarity") {
      ...
      var match = config[group].filter(function (opt) {
  ...
  function renderChips() {
    ...
    if (activeFiltersLabel)
      activeFiltersLabel.classList.toggle("d-none", chips.length === 0);

    chipTrack.innerHTML = chips
      .map(function (chip) {
        return (
          '<span class="filter-chip" data-group="' +
          chip.group +
          '">' +
          chip.label +
  ...
  function openProductModal(id) {
    ...
    if (product.image) {
      ...
      modalImage.className =
        "modal-product-image " + (product.art || "card-art-1");
    ...
    var requirements = Array.isArray(product.requirements)
      ? product.requirements.filter(function (requirement) {
    ...
    modalCondition.textContent =
      ...
      FILTER_LABELS.condition[product.condition] ||
      product.condition ||
      "Not specified";
  ...
  function addActiveProductToCart() {
    ...
    fetch("add_to_cart.php", {
      ...
      .then(function (result) {
        if (!result.success) {
          showToast(
            result.message || "Could not add this to your cart.",
            "error",
          );
