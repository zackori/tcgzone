...
function renderOrders(orders) {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(orders.length / PROCUREMENT_PER_PAGE),
  );
  procurementPage = Math.min(procurementPage, totalPages);
  const pageOrders = orders.slice(
    (procurementPage - 1) * PROCUREMENT_PER_PAGE,
    procurementPage * PROCUREMENT_PER_PAGE,
  );
  document.getElementById("procurementPage").textContent = procurementPage;
  document.getElementById("previousProcurementPage").disabled =
    procurementPage === 1;
  document.getElementById("nextProcurementPage").disabled =
    procurementPage === totalPages;
  if (!orders.length) {
    table.innerHTML =
      '<tr><td colspan="5" class="text-center">No procurement receipts found.</td></tr>';
    return;
  }
  table.innerHTML = pageOrders
    .map(
      (order) => `
        <tr class="order-row-clickable" data-order-id="${Number(order.procurement_order_id)}">
            <td>${Number(order.procurement_order_id)}</td><td>${escapeHtml(order.supplier_name)}</td><td>${currency(order.display_total_amount ?? order.total_amount)}</td>
...
function renderFilteredOrders() {
  const keyword = document
    .getElementById("searchProcurement")
    .value.trim()
    .toLowerCase();
...
async function loadProcurement() {
  try {
    ...
    document.getElementById("buyRequestCount").textContent =
      procurementOrders.filter(
        (order) => order.order_status === "Pending",
      ).length;
    document.getElementById("successBuyCount").textContent =
      procurementOrders.filter(
    ...
    document.getElementById("procurementTable").innerHTML =
      '<tr><td colspan="5" class="text-center">Could not load procurement receipts.</td></tr>';
  }
}
...
function renderLowStock(products) {
  ...
  if (!products.length) {
    table.innerHTML =
      '<tr><td colspan="10" class="text-center">No cards are low in stock.</td></tr>';
    return;
  }
  table.innerHTML = products
    ...
    .join("");
  table.querySelectorAll(".resupply-checkbox").forEach((checkbox) =>
    checkbox.addEventListener("change", () => {
      checkbox.closest("tr").querySelector(".resupply-quantity").disabled =
        !checkbox.checked;
    }),
  );
}
...
async function loadLowStock(showModal = true) {
  ...
  try {
    ...
    if (!response.ok || !result.success)
      throw new Error(result.message || "Could not load low-stock cards.");
    lowStockProducts = result.products;
    document.getElementById("stockCount").textContent = lowStockProducts.length;
    document.getElementById("supplierSelect").innerHTML = result.suppliers
      .map(
        (supplier) =>
          `<option value="${Number(supplier.supplier_id)}">${escapeHtml(supplier.supplier_name)}</option>`,
      )
      .join("");
    renderLowStock(lowStockProducts);
    if (showModal)
      document.getElementById("lowStockModal").classList.remove("d-none");
  } catch (error) {
...
function addPurchasedCard() {
  ...
  list.insertAdjacentHTML(
    "beforeend",
    purchasedCardTemplate(purchasedCardCount),
  );
...
function openPurchasedCardsModal() {
  ...
  document.getElementById("purchasedCardsMessage").className =
    "modal-msg d-none";
  addPurchasedCard();
  document.getElementById("purchasedCardsModal").classList.remove("d-none");
}

function closePurchasedCardsModal() {
  document.getElementById("purchasedCardsModal").classList.add("d-none");
}
...
async function insertPurchasedCards(event) {
  ...
  try {
    sections.forEach((section, index) => {
      const values = Object.fromEntries(
        [...section.querySelectorAll("input[name], select[name]")].map(
  ...
  message.textContent = "Adding purchased card(s)...";
  try {
    const response = await fetch("api/add-purchased-cards.php", {
      method: "POST",
      body: formData,
    });
    const result = await response.json();
    if (!response.ok || !result.success)
      throw new Error(result.message || "Could not add purchased cards.");
    closePurchasedCardsModal();
    await loadProcurement();
    await loadLowStock(false);
  } catch (error) {
    message.className = "modal-msg error";
    message.textContent = error.message || "Could not add purchased cards.";
  } finally {
...
async function placeProcurementOrder() {
  const rows = [
    ...document.querySelectorAll("#lowStockTable tr[data-product-id]"),
  ];
  const items = rows
    .filter((row) => row.querySelector(".resupply-checkbox").checked)
    .map((row) => ({
      product_id: Number(row.dataset.productId),
      quantity: Number(row.querySelector(".resupply-quantity").value),
    }));
  const message = document.getElementById("lowStockMessage");
  message.className = "modal-msg";
  message.textContent = "Placing supplier order...";
  try {
    const response = await fetch("api/place-procurement-order.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        supplier_id: Number(document.getElementById("supplierSelect").value),
        items,
      }),
    });
    const result = await response.json();
    if (!response.ok || !result.success)
      throw new Error(result.message || "Could not place procurement order.");
    document.getElementById("lowStockModal").classList.add("d-none");
    await loadProcurement();
  } catch (error) {
    message.className = "modal-msg error";
    message.textContent = error.message || "Could not place procurement order.";
  }
}
...
async function openProcurementDetails(orderId) {
  ...
  document.getElementById("procurementDetailsTitle").textContent =
    "Loading receipt...";
  try {
    const response = await fetch(
      `api/get-procurement-items.php?procurement_order_id=${encodeURIComponent(orderId)}`,
    );
    const result = await response.json();
    if (!response.ok || !result.success)
      throw new Error(result.message || "Could not load receipt.");
    activeProcurementOrder = result.order;
    document.getElementById("procurementDetailsTitle").textContent =
      `Procurement Receipt #${result.order.procurement_order_id}`;
    document.getElementById("procurementSupplier").textContent =
      result.order.supplier_name;
    const status = document.getElementById("procurementStatus");
    status.className = `modal-status-badge ${statusClass(result.order.order_status)}`;
    status.textContent = result.order.order_status;
    document.getElementById("procurementDate").textContent =
      result.order.order_date;
    document.getElementById("procurementItemsTotal").textContent = currency(
      result.order.items_subtotal || 0,
    );
    document.getElementById("procurementShippingFee").textContent = currency(
      result.order.shipping_fee || 0,
    );
    document.getElementById("procurementTotal").textContent = currency(
      result.order.total_amount,
    );
    document.getElementById("procurementStatusSelect").value =
    ...
    document.getElementById("procurementDetailsItems").innerHTML =
      result.items
        .map(
          (item) =>
            `<tr><td>${escapeHtml(item.card_name)}<br><small>${escapeHtml(item.set_name)}</small></td><td>${Number(item.quantity)}x</td><td>${currency(item.unit_cost)}</td><td>${currency(item.subtotal)}</td></tr>`,
        )
        .join("") ||
      '<tr><td colspan="4" class="modal-empty-msg">No cards found for this receipt.</td></tr>';
  } catch (error) {
    document.getElementById("procurementDetailsMessage").className =
      "modal-msg error";
    document.getElementById("procurementDetailsMessage").textContent =
...
async function updateProcurementStatus() {
  ...
  try {
    const response = await fetch("api/update-procurement-status.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        procurement_order_id: activeProcurementOrder.procurement_order_id,
        status,
      }),
    });
    const result = await response.json();
    ...
    if (status === "Delivered") {
      ...
      message.textContent =
        "Delivered. The ordered quantities were added to product stock.";
    }
  } catch (error) {
    message.className = "modal-msg error";
...
document
  .getElementById("procurementStatusFilter")
  .addEventListener("change", filterOrders);
document
  .getElementById("lowStockCard")
  .addEventListener("click", openLowStockModal);
document.getElementById("buyRequestCard").addEventListener("click", () => {
  document.getElementById("procurementStatusFilter").value = "Pending";
  filterOrders();
});
document.getElementById("successBuyCard").addEventListener("click", () => {
  document.getElementById("procurementStatusFilter").value = "Delivered";
  filterOrders();
});
[
  ["lowStockCard", openLowStockModal],
  [
    "buyRequestCard",
    () => {
      document.getElementById("procurementStatusFilter").value = "Pending";
...
document
  .getElementById("closeLowStockModal")
...
document
  .getElementById("placeProcurementOrder")
  .addEventListener("click", placeProcurementOrder);
document
  .getElementById("addPurchasedCards")
  .addEventListener("click", openPurchasedCardsModal);
document
  .getElementById("closePurchasedCardsModal")
  .addEventListener("click", closePurchasedCardsModal);
document
  .getElementById("purchasedCardsModal")
  .addEventListener("click", (event) => {
    if (event.target.id === "purchasedCardsModal") closePurchasedCardsModal();
  });
document
  .getElementById("addAnotherPurchasedCard")
  .addEventListener("click", addPurchasedCard);
document
  .getElementById("purchasedCardsList")
  .addEventListener("change", (event) => {
    if (!event.target.classList.contains("purchased-image-input")) return;
    const file = event.target.files[0];
    if (!file) return;
    const preview = event.target
      .closest(".purchased-card-form")
      .querySelector(".purchased-image-preview");
...
document
  .getElementById("purchasedCardsForm")
  .addEventListener("submit", insertPurchasedCards);
document
  .getElementById("procurementTable")
  .addEventListener("click", (event) => {
    const row = event.target.closest("tr[data-order-id]");
    if (row) openProcurementDetails(row.dataset.orderId);
  });
document
  .getElementById("closeProcurementDetailsModal")
  .addEventListener("click", () =>
    document.getElementById("procurementDetailsModal").classList.add("d-none"),
  );
document
  .getElementById("procurementDetailsModal")
  .addEventListener("click", (event) => {
    if (event.target.id === "procurementDetailsModal")
      event.currentTarget.classList.add("d-none");
  });
document
  .getElementById("updateProcurementStatus")
  .addEventListener("click", updateProcurementStatus);
document
  .getElementById("previousProcurementPage")
  .addEventListener("click", () => {
    if (procurementPage > 1) {
      procurementPage -= 1;
      renderFilteredOrders();
    }
  });
document.getElementById("nextProcurementPage").addEventListener("click", () => {
  procurementPage += 1;
  renderFilteredOrders();
});
...
loadProcurement();
loadLowStock(
  new URLSearchParams(window.location.search).get("low_stock") === "1",
);
