...
try {
    ...
    // Match on the exact card attributes agreed for duplicate prevention.
    // Product type is intentionally not included: the requested matching rule is
    // card name, set, rarity, category, and condition.
    $matchStmt = mysqli_prepare(
        $conn,
        'SELECT product_id
         FROM products
         WHERE card_name = ? AND set_name = ? AND rarity = ? AND category = ? AND `condition` = ?
         LIMIT 1
         FOR UPDATE'
    );
    ...
    $existingProduct = mysqli_fetch_assoc($matchResult);
    ...
    try {
        ...
        // Return the original error as JSON even if the connection is already invalid.
    }
    http_response_code(500); echo json_encode(['success' => false, 'message' => $error->getMessage()]);
