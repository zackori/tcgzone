...
$sellCostQuery = "SELECT COALESCE(SUM(selling_price * stock_quantity), 0) AS total FROM sell_requests WHERE status = 'Approved'";
$sellRequestCost = (float) mysqli_fetch_assoc(mysqli_query($conn, $sellCostQuery))['total'];

$procurementCostQuery = "SELECT COALESCE(SUM(item.subtotal), 0) AS total FROM procurement_orders purchase JOIN procurement_order_items item ON item.procurement_order_id = purchase.procurement_order_id WHERE purchase.order_status = 'Delivered'";
$procurementCost = (float) mysqli_fetch_assoc(mysqli_query($conn, $procurementCostQuery))['total'];
...
$sellMonthlyCostQuery = "SELECT YEAR(created_at) AS year, MONTH(created_at) AS month, COALESCE(SUM(selling_price * stock_quantity), 0) AS cost FROM sell_requests WHERE status = 'Approved' GROUP BY YEAR(created_at), MONTH(created_at)";
$sellMonthlyCosts = mysqli_query($conn, $sellMonthlyCostQuery);
while ($row = mysqli_fetch_assoc($sellMonthlyCosts)) {
    if ((string) $row['year'] === (string) $selectedYear) $monthlyCosts[(int) $row['month'] - 1] += (float) $row['cost'];
}

$procurementMonthlyCostQuery = "SELECT YEAR(purchase.order_date) AS year, MONTH(purchase.order_date) AS month, COALESCE(SUM(item.subtotal), 0) AS cost FROM procurement_orders purchase JOIN procurement_order_items item ON item.procurement_order_id = purchase.procurement_order_id WHERE purchase.order_status = 'Delivered' GROUP BY YEAR(purchase.order_date), MONTH(purchase.order_date)";
