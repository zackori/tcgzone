document.addEventListener("DOMContentLoaded", function () {
  ...
  function getDetails(id) {
    return detailsById[id] || { description: "", requirements: [] };
  }
  function getStockStatus(stock) {
    return stock <= 0
      ? { key: "out-of-stock", label: "Out of Stock" }
      : stock <= 3
        ? { key: "low-stock", label: "Low on Stock" }
  ...
  function loadDetails() {
    return fetch("../../customer/Shop%20Page/products.json")
      .then(function (r) {
        if (!r.ok) throw new Error("Could not load product details.");
        return r.json();
      })
      .then(function (products) {
        detailsById = {};
        products.forEach(function (product) {
          detailsById[product.id] = {
  ...
  function loadInventory() {
    ...
    tableBody.innerHTML =
      '<tr><td colspan="8" class="text-center">Loading inventory…</td></tr>';
    fetch("api/inventory_api/get_products.php?" + params)
      .then(function (r) {
        return r.json();
      })
      ...
      .catch(function () {
        tableBody.innerHTML =
          '<tr><td colspan="8" class="text-center">Could not load inventory.</td></tr>';
      });
  }
  function renderTable(products) {
    if (!products.length) {
      tableBody.innerHTML =
        '<tr><td colspan="8" class="text-center">No cards match this search/filter.</td></tr>';
      return;
    }
    tableBody.innerHTML = products
      .map(function (p) {
        var status = p.isArchived
            ? { key: "archived", label: "Archived" }
            : getStockStatus(p.stockQuantity),
          img = p.image || "/tcgzone/assets/images/admin/no-image.png";
        return (
          "<tr><td>" +
          escapeHtml(p.productId) +
          '</td><td><img class="table-thumb" src="' +
          escapeHtml(img) +
          '" alt=""></td><td>' +
  ...
  function addRequirement(item) {
    ...
    row.innerHTML =
      '<input class="form-control requirement-label" placeholder="Detail label"><textarea class="form-control requirement-value" rows="3" placeholder="Detail value"></textarea><button class="remove-requirement-btn" type="button" aria-label="Remove detail">×</button>';
    row.querySelector(".requirement-label").value = item.label || "";
    row.querySelector(".requirement-value").value = item.value || "";
  ...
  function setRequirements(items) {
    editor.innerHTML = "";
    (items && items.length ? items : [{ label: "", value: "" }]).forEach(
      addRequirement,
    );
  }
  function requirementsFromForm() {
    return Array.from(editor.querySelectorAll(".requirement-row"))
      .map(function (row) {
        return {
          label: row.querySelector(".requirement-label").value.trim(),
  ...
  function openCardModal(mode, product) {
    ...
    document.getElementById("cardCategory").value = product
      ? product.category
      : "Pokémon";
    document.getElementById("cardProductType").value = product
      ? product.productType
      : "Cards";
    document.getElementById("cardRarity").value = product ? product.rarity : "";
    document.getElementById("cardCondition").value = product
      ? product.condition
      : "";
    document.getElementById("cardPrice").value = product ? product.price : "";
    document.getElementById("cardStock").value = product
      ? product.stockQuantity
      : "";
    document.getElementById("cardDescription").value = details.description;
    setRequirements(details.requirements);
    var hasImage = product && product.image;
    document
      .getElementById("existingImageWrap")
    ...
    document.getElementById("cardModalBackdrop").classList.add("show");
  }
  ...
  document
    .getElementById("addRequirementBtn")
    .addEventListener("click", function () {
      addRequirement();
    });
  document
    .getElementById("cardFormCancelBtn")
    .addEventListener("click", function () {
      closeModal("cardModalBackdrop");
    });
  document
    .getElementById("cardImageInput")
    .addEventListener("change", function () {
      ...
      if (
        ...
        alert("Use a JPG or PNG smaller than 1MB.");
        this.value = "";
        return;
      }
      ...
      reader.onload = function (e) {
        ...
        document.getElementById("existingImageWrap").classList.remove("d-none");
        document.getElementById("uploadField").classList.add("d-none");
      };
  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      document.getElementById("existingImageWrap").classList.add("d-none");
      document.getElementById("uploadField").classList.remove("d-none");
    });
  tableBody.addEventListener("click", function (e) {
    var edit = e.target.closest('[data-action="edit"]'),
      del = e.target.closest('[data-action="delete"]'),
      restore = e.target.closest('[data-action="restore"]');
    if (edit) {
      fetch(
        "api/inventory_api/get_products.php?id=" +
          encodeURIComponent(edit.dataset.id),
      )
        .then(function (r) {
          return r.json();
        })
        .then(function (result) {
          if (result.success && result.products.length)
            openCardModal("edit", result.products[0]);
        });
    }
    if (del) {
      pendingDeleteId = del.dataset.id;
      pendingDeleteName = del.dataset.name;
      document.getElementById("deleteCardName").textContent = pendingDeleteName;
  ...
  document.getElementById("cardForm").addEventListener("submit", function (e) {
    ...
    fd.append("imageRemoved", imageWasRemoved ? "1" : "0");
    if (selectedImageFile) fd.append("image", selectedImageFile);
    var btn = document.getElementById("cardFormSaveBtn");
    btn.disabled = true;
    fetch(
      isEdit
        ? "api/inventory_api/update_product.php"
        : "api/inventory_api/add_product.php",
      { method: "POST", body: fd },
    )
      .then(function (r) {
        return r.json();
      ...
      .then(function (result) {
        if (!result.success) throw new Error(result.message);
        var productId = isEdit ? id : result.productId;
        return fetch("api/inventory_api/update_description.php", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            productId: productId,
            description: document
              .getElementById("cardDescription")
              .value.trim(),
  ...
  document
    ...
    .addEventListener("click", function () {
      closeModal("successModalBackdrop");
    });
  document
  ...
  document
    .getElementById("archiveFilter")
    .addEventListener("change", function () {
      state.archive = this.value;
      state.page = 1;
      loadInventory();
    });
  document
