...
function displaySellRequests(requests) {
    ...
    tbody.innerHTML = requests.map((request) => `
        <tr class="order-row-clickable" data-request-id="${Number(request.request_id)}">
            ...
            <td>${formatCurrency(Number(request.selling_price || 0) * Number(request.quantity || 0))}</td>
...
async function loadSellRequests() {
    try {
        ...
        console.error(error);
        document.getElementById("sellRequestsTable").innerHTML = '<tr><td colspan="7" class="text-center">Could not load sell requests. Please try again.</td></tr>';
    }
}
